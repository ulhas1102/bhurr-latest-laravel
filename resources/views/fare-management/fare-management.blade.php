                ...
                <table class="table display" style="width:100%" id="example">
                  <thead>
                    <tr>
                        ...
                        <th>Outstation P Cng Per km price</th>
                        <th>Outstation P Diesel Per km price</th>
                        <th>One Way Cng Per km price</th>
                        <th>One Way Diesel Per km price</th>
                        <th>Extra Hour Range</th>
                        <th>Per Extra Hour Charges</th>
						            <th>Local Base Fare</th>
                        <th>Outstation Base Fare</th>
                  ...
                  <tbody>
                   ...
                   @foreach($carclasses as $carclass)
                    ...
                    <td>
                      @if($carclass->outstation_cng_per_km_charges != null)
                      {{$carclass->outstation_cng_per_km_charges}}
                      @else
                      - 
                     @endif
                    </td>
                    <td>
                      @if($carclass->outstation_disel_per_km_charges != null)
                      {{$carclass->outstation_disel_per_km_charges}}
                      @else
                      - 
                     @endif
                    </td>
                    <td>
                      @if($carclass->oneway_cng_per_km_charges != null)
                      {{$carclass->oneway_cng_per_km_charges}}
                      @else
                      - 
                     @endif
                    </td>
                    <td>
                      @if($carclass->oneway_disel_per_km_charges != null)
                      {{$carclass->oneway_disel_per_km_charges}}
                      @else
                      - 
                     @endif
                    </td>
...
<div class="modal fade" id="editPaymentHistoryModal" tabindex="-1" role="dialog" aria-labelledby="editPaymentHistoryModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
      <div class="modal-content">
          ...
          <div class="modal-body">
              ...
              <div class="form-group">
                <label for="editOutstationDiselPerKMCharges">Outstation package Disel Per Km Charges<small>(In Rupees)</small></label>
                <input type="text" class="form-control" id="editOutstationDiselPerKMCharges" name="outstation_disel_per_km_charges" required>
            </div>
            <div class="form-group">
              <label for="editOutstationCngPerKMCharges">Outstation package CNG Per Km Charges<small>(In Rupees)</small></label>
              <input type="text" class="form-control" id="editOutstationCngPerKMCharges" name="outstation_cng_per_km_charges" required>
          </div>
          <div class="form-group">
            <label for="editOnewayDiselPerKMCharges">One way Disel Per Km Charges<small>(In Rupees)</small></label>
            <input type="text" class="form-control" id="editOnewayDiselPerKMCharges" name="oneway_disel_per_km_charges" required>
        </div>
        <div class="form-group">
          <label for="editOnewayCngPerKMCharges">One way CNG Per Km Charges<small>(In Rupees)</small></label>
          <input type="text" class="form-control" id="editOnewayCngPerKMCharges" name="oneway_cng_per_km_charges" required>
      </div>
              <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="editOutstationBaseFare" name="outstation_base_fare" required>
              </div>
              <div class="form-group">
  ...
  <script>
    ...
    document.getElementById('editOutstationDiselPerKMCharges').value = carclass.outstation_disel_per_km_charges;
	  document.getElementById('editOutstationCngPerKMCharges').value = carclass.outstation_cng_per_km_charges;
    document.getElementById('editOnewayDiselPerKMCharges').value = carclass.oneway_disel_per_km_charges;
	  document.getElementById('editOnewayCngPerKMCharges').value = carclass.oneway_cng_per_km_charges;
