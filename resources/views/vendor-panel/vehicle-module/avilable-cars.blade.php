...
<div class="main-panel">
        ...
        <div class="card">
            <div class="card-header d-flex  justify-content-between p-3">
              <h4>Available Cars list</h4>
                <a href="{{url('add-cars')}}" class="btn btn-primary">ADD CAR</a>
            </div>
            <div class="card-body" style="overflow-y: auto; max-height: 400px;">
                <table class="table display" style="width:100%" id="example2">
                   ...
                   @foreach($vendorcars as $vendorcar)
                    <tr>
                    <td>{{$i++}}</td>
                    <td>{{$vendorcar->car_name}}</td>
                    <td>{{$vendorcar->car_type}}</td>
                    <td>{{$vendorcar->car_class}}</td>
                    <td>{{$vendorcar->car_registration}}</td>
                    <td>{{$vendorcar->no_of_cars_quantity}}</td>
                    <td class="d-flex justify-content-between">
                        <a href="vendor-car-details?car_details_id={{$vendorcar->id}}" class="btn btn-primary">View</a>
                      <button type="button" class="btn btn-primary" style="
                      padding-left: 18px;
                      padding-right: 18px;
                      padding-bottom: 3px;
                      height: 36.333334px;" 
                      onclick="editUploadDocument({{ json_encode($vendorcar) }})">
                      Upload Document
                  </button>
                        <form action="deleteCar" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{$vendorcar->id}}">
  ...
  <script>
   function editUploadDocument(vendorcar) {
    // Populate the form fields in the modal with the provided car details
    document.getElementById('editSalesTaxreceipt').src = `./car-details/${vendorcar.sales_tax_receipt}`;
    document.getElementById('editProfessionalTaxreceipt').src = `./car-details/${vendorcar.professional_tax_receipt}`;
    document.getElementById('editRTOtaxreceipt').src = `./car-details/${vendorcar.rto_tax_receipt}`;
    document.getElementById('editAuthorizationletter').src = `./car-details/${vendorcar.authorization_letter}`;
    document.getElementById('editFitnessCertificate').src = `./car-details/${vendorcar.fitness_certificate}`;
    document.getElementById('editPUC').src = `./car-details/${vendorcar.puc}`;
    document.getElementById('editInsurancePolicy').src = `./car-details/${vendorcar.insurance_policy}`;
    document.getElementById('editRCBookImage').src = `./car-details/${vendorcar.rc_book}`;
    document.getElementById('editUploadDocumentsForm').action = `./edit-upload-vendor-cars-document/${vendorcar.id}`;
