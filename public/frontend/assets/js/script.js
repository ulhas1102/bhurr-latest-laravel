...
$(document).ready(function () {
    $(".read__more").click(function () {
        ...
        var container = $(this).closest('.specific__car__card').find('.inclusion__exclusion__container');
        container.slideToggle();

        // Toggle text of the button
        $(this).find('.read__more').text(function (i, text) {
...
$(document).ready(function () {
    $(".fa-angle-down, .fa-angle-up").click(function () {
        ...
        $(this).closest('.departure__card, .booking__review__container, .passengers__details').find('.summary-content').slideToggle();
...
$(document).ready(function () {
    let mobileNumberCount = 0; // Start the count from 0
    $('#addMobileNumberBtn').click(function (event) {
        event.preventDefault(); // Prevent the default form submission
        mobileNumberCount++;
        $('#additionalMobileNumbers').after(`
            <div class="col-md-4 form-group">
                <input type="text" class="form-control input" id="mobno${mobileNumberCount}" name="contactmobile[${mobileNumberCount}]" required>
                <label for="mobno${mobileNumberCount}" class="user__label">Enter Mobile number ${mobileNumberCount}*</label>
            </div>
        `);
    });
});
...
window.addEventListener('resize', function () {
    ...
    if (window.innerWidth <= 767 && brElement) {
        ...
        if (h2Element) {
            ...
            h2Element.innerHTML = h2Element.innerHTML.replace('TO TRAVEL', '') + '<br class="mobile-hide"> TO TRAVEL';
        }
    }
});
...
$(document).ready(function () {
    ...
    function checkHeightAndToggleScroll() {
        if ($('#searchTab').hasClass('show active') && $('#searchTab.outer__tab__content').height() > 205) {
